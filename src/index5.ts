 ...
 * @LastEditors: Huangjs
 * @LastEditTime: 2023-05-22 11:47:34
 * @Description: ******
 */
...
const touchMove = function touchMove(this: ImageView, e: GEvent) {
  if (this._startMoving) {
    ...
    const a = adjustAngle(sa + ma, rotation, true);
    const k = adjustScale(sk * mk, scalation, true);
    const d = 1 - k / sk;
    const translationX = translation[0] || this.getBoundary(k, 'x');
    const x = adjustXY(sx + mx + offset[0] * d, translationX, true);
    const translationY = translation[1] || this.getBoundary(k, 'y');
    const y = adjustXY(sy + my + offset[1] * d, translationY, true);
...
const touchEnd = function touchEnd(this: ImageView) {
  if (this._startMoving) {
    ...
    if (this._animation.length === 0) {
      ...
      const { a: sa = 0, k: sk = 1, x: sx = 0, y: sy = 0 } = sTransform;
      const { rotation, scalation, translation } = this;
      const a = adjustAngle(sa + ma, rotation, false);
      const k = adjustScale(sk * mk, scalation, false);
      const d = 1 - k / sk;
      const translationX = translation[0] || this.getBoundary(k, 'x');
      const x = adjustXY(sx + mx + offset[0] * d, translationX, false);
      const translationY = translation[1] || this.getBoundary(k, 'y');
      const y = adjustXY(sy + my + offset[1] * d, translationY, false);
...
const doubleTap = function doubleTap(this: ImageView, e: GEvent) {
  ...
  if ((dk > 1 && tk <= 1) || (dk < 1 && tk >= 1)) {
    ...
    const x = adjustXY(tx + ox * d, translationX, false);
    const translationY = translation[1] || this.getBoundary(dk, 'y');
    const y = adjustXY(ty + oy * d, translationY, false);
        ...
        .then(() => {
          ...
          if (!inBoundary) {
            transition.apply(this, [
              true,
              { x: adjustXY(swipeX, translationX, false) },
            ]);
          }
        });
    } else {
      // 已经超出边界，不再进行惯性位移，直接归位
      transition.apply(this, [true, { x: adjustXY(x, translationX, false) }]);
    }
      ...
      transition
        ...
        .then(() => {
          ...
          if (!inBoundary) {
            transition.apply(this, [
              true,
              { y: adjustXY(swipeY, translationY, false) },
            ]);
          }
        });
    } else {
      // 如果已经在范围之外，就不再进行惯性位移，直接归位
      transition.apply(this, [true, { y: adjustXY(y, translationY, false) }]);
    }
  }
};
