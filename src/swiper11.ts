 ...
 * @LastEditors: Huangjs
 * @LastEditTime: 2023-07-07 10:13:40
 * @Description: ******
 */
...
const touchMove = function touchMove(this: Swiper, e: GEvent) {
  ...
  if (image) {
    ...
    const transformRaw = { a: ta, k: tk, x: tx, y: ty };
    if (this._fingers === 1) {
      ...
      if (xRange[0] < 0 && xRange[1] > 0) {
        ...
        if (image.isDamping('translate')) {
          ...
          adjustX = performDamping;
        }
        transformRaw.x = adjustX(_tx + deltaX, xRange);
      }
      ...
      if (yRange[0] < 0 && yRange[1] > 0) {
        ...
        if (image.isDamping('translate')) {
          ...
          adjustY = performDamping;
        }
        transformRaw.y = adjustY(_ty + deltaY, yRange);
      }
    } else {
      ...
      // 把原值进行各项变化，再进行总体阻尼计算
      transformRaw.a = adjustRotate(_ta + angle, image.getRotation());
      const k = (transformRaw.k = adjustScale(
        _tk * scale,
        image.getScalation(),
        true,
      ));
      const [ox, oy] = image.computeOffset(point, k);
      transformRaw.x = adjustTranlate(
        _tx + ox + deltaX,
        image.getXTranslation(k),
      );
      transformRaw.y = adjustTranlate(
        _ty + oy + deltaY,
        image.getYTranslation(k),
      );
    }
...
class Swiper {
  ...
  constructor({
    ...
    this._images = imageUrls.map((url, i) => {
      const wrapper = setStyle(document.createElement('div'), {
        ...
        overflow: 'hidden',
        flex: 1,
        marginLeft: i === 0 ? 0 : this._columnGap,
      });
  ...
  createImage(index: number) {
    this._activeIndex = index;
    const imageObj = this._images[index];
  ...
  resize() {
    ...
    setStyle(this.contentEl, {
      width: width * len + (len - 1) * this._columnGap,
      height,
    });
  ...
  slide(index: number, duration?: number) {
    const _index = Math.min(Math.max(index, 0), this._images.length - 1);
    const _translate = -_index * (this._eleSize[0] + this._columnGap);
    this.transitionRun(_translate, duration).then(() => {
      // resize上一个
      this.resize();
      // 创建下一个
      this.createImage(_index);
    });
  }
  next() {
  ...
  transitionRun(translate: number, duration: number = 4000) {
