...
import Transition, { TAProperty, easeOutQuad, easeOutQuart } from '@huangjs888/transition';
...
class Entity {
  ...
  transform(transformRaw, point, options) {
    const {
      a: ta = 0,
      k: tk = 1,
      x: tx = 0,
  ...
  transformTo(transformRaw, point, options) {
    let _point = point;
  ...
  transitionRun(transformRaw, options = {}) {
    ...
    return this._transition.apply(deltaValue, _extends({
      precision,
      cancel: true,
      duration: 500,
      easing: easeOutQuart
  ...
  computeOffset(point, k, adjust = false) {
    const {
      containerCenter,
      containerWidth,
      containerHeight,
    ...
    if (adjust) {
      ...
      ox = between(ew - (cw - ew) / (dk - 1), [0, ew]) * ratioOffset(ox / ew, dk, between(cw / ew, [1, 2])) || 0;
      oy = between(eh - (ch - eh) / (dk - 1), [0, eh]) * ratioOffset(oy / eh, dk, between(ch / eh, [1, 2])) || 0;
    }
    ox *= 1 - dk;
    oy *= 1 - dk;
    return [ox, oy];
  }
  moveBounce(angle, scale, deltaX, deltaY, point = []) {
    let {
      a = 0,
      k = 1,
      x = 0,
    ...
    if (this.isDamping('rotate')) {
      ...
      a = ba + revokeDamping(a - ba, {
        max: 180
      });
      // 再对总值进行总体阻尼计算
      ba = between(a += angle, aRange);
      a = ba + performDamping(a - ba, {
        max: 180
      });
    } else {
    ...
    if (this.isDamping('scale')) {
      ...
      k = bk * revokeDamping(k / bk, {
        max: 2,
        mode: 1
      });
      // 再对总值进行总体阻尼计算
      bk = between(k *= scale, kRange);
      k = bk * performDamping(k / bk, {
        max: 2,
        mode: 1
      });
    } else {
  ...
  resetBounce(point = [], cancel = false) {
    let {
      a = 0,
      k = 1,
      x = 0,
  ...
  swipeBounce(duration, stretch, key, transition) {
    ...
    if (isBetween(xyPos + _stretch, xyRange)) {
      ...
      if (this.isDamping('translate')) {
        ...
        const velocity = (isBetween(xyPos, xyRange) ? Math.sqrt(1 - Math.abs(xySwipe / _stretch)) : 1) * (2 * Math.abs(_stretch) / _duration);
      ...
      if (typeof transition === 'function') {
        ...
        transition(key, xySwipe, xyBounce, xyMove => {
        ...
        if (xyMove === 0) {
          ...
          this.transitionRun({
            [key]: between(xyPos + xyMove, xyRange)
          });
        } else {
          // 先移动xyMove距离
          this.transitionRun({
